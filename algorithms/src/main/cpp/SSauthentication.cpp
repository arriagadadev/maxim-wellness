...
void mxm_sha256_transform(MXM_SHA256_CTX *ctx, const BYTE data[])
{
...
void mxm_sha256_update(MXM_SHA256_CTX *ctx, const BYTE data[], int len)
{
	...
	for (i = 0; i < len; ++i) {
		...
		ctx->datalen++;
		if (ctx->datalen == 64) {
			mxm_sha256_transform(ctx, ctx->data);
			ctx->bitlen += 512;
			ctx->datalen = 0;
		}
	}
...
void shuffleDeshuffle( const BYTE* const input , BYTE* output , const int iolen ) {
    ...
    const BYTE KEY[] = { 0x33, 0x43, 0x47, 0x66, 0x32, 0x76, 0x44, 0x53, 0x73, 0x74, 0x65, 0x45, 0x67, 0x52, 0x73, 0x64, 0x76, 0x67};

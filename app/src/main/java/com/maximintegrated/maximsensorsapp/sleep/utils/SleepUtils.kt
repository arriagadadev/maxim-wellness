...
fun postProcessing(sleepList: List<Sleep>) {
        ...
        if(sleep.latency > 0){
            for(j in 0 until sleep.latency * 60){
                if(i - j > 0){
                    ...
                    sleepList[i-j].sleepPhasesOutput = SleepAlgorithmOutput.SleepPhaseOutput.LIGHT.value
                }
            }
        }
    }
        ...
        if(sleepList[i].sleepPhasesOutputProcessed == SleepAlgorithmOutput.SleepPhaseOutput.UNDEFINED.value){
            ...
            for(k in 0 until j){
                if((k < j / 2 && i > 1) //if k belongs to first half and there is a previous value
                    || (i + j == sleepList.size)){ //if counter reached to the end of array use only previous value
                    //fill the first half with previous value
                    sleepList[i + k].sleepPhasesOutputProcessed = sleepList[i - 1].sleepPhasesOutputProcessed
                }else{
                    //fill the second half with next value
                    sleepList[i + k].sleepPhasesOutputProcessed = sleepList[i + j].sleepPhasesOutputProcessed
                }
            }
